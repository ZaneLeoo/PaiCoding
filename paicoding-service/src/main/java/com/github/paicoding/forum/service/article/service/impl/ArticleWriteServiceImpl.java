...
public class ArticleWriteServiceImpl implements ArticleWriteService {
    ...
    public Long saveArticle(ArticlePostReq req, Long author) {
        ArticleDO article = ArticleConverter.toNewArticleDO(req, author);
            ...
            public Long doInTransaction(TransactionStatus status) {
                ...
                if (NumUtil.nullOrZero(req.getArticleId())) {
                    ...
                    req.getTags().forEach(tag -> {
                        tagIds.add(ArticleTagEnum.fromDescription(tag));
                    });
                    articleId = insertArticle(article, content, tagIds);
...
//                return articleId;
     ...
     * @return
     */
    private Long insertArticle(ArticleDO article, ArticleContentDO content, Set<Long> tags) {
        // article + article_content + tag  三张表的数据变更
        // 1. 保存文章
        // 使用分布式id生成文章主键
        ...
        // 3. 保存文章标签
        List<ArticleTagsDO> tagList = ArticleConverter.toArticleTagsDOList(tags, articleId);
        articleTagsDao.saveBatch(tagList);
...
//        }
    ...
    public void addArticleViews(Long articleId) {
        ArticleDO article = articleDao.getArticleById(articleId);
...
//        return article.getStatus() == PushStatusEnum.ONLINE.getCode() && !articleWhiteListService.authorInArticleWhiteList(article.getUserId());
    ...
    @Autowired
    public void setArticleWhiteListService(AuthorWhiteListService articleWhiteListService) {
        this.articleWhiteListService = articleWhiteListService;
    }
}
